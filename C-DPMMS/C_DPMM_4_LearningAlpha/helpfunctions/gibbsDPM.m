function [partition_samples, partition_estimate, similarity,p_mu,p_sig,p_isig,ind1,cluster_size] = gibbsDPM(Data,da,dw,y, hyperG0, alpha, niter, type_algo, doPlot,learning)
...
switch(type_algo)
    ...
    case 'CRP' % Based on the Chinese Restaurant Process
        [partition_samples, MU, SIG, iSIG] = gibbsDPM_algo2(Data,da,dw,y,hyperG0, alpha, niter, doPlot,learning);
...
[partition_estimate,cost,similarity,ind,ind1,cluster_size] = cluster_est_binder(partition_samples);
dim=size(MU,1)/(niter/2);
s1=(ind-1)*dim+1;s2=s1+dim-1;
p_mu=MU(s1:s2,:);p_sig=SIG(s1:s2,:,:);p_isig=iSIG(s1:s2,:,:);
